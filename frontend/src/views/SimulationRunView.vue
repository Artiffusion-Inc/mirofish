...
// Get maxRounds from query params at init time, ensuring child component receives the value immediately
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        if (configRes.success && configRes.data?.time_config?.minutes_per_round) {
          ...
          addLog(`Time config: ${minutesPerRound.value} minutes`)
        }
      } catch (configErr) {
...
const loadGraph = async (graphId) => {
  // During simulation, auto-refresh does not show full-screen loading to avoid flicker
...
onMounted(() => {
  ...
  // Record maxRounds config (value obtained from query params at init)
  if (maxRounds.value) {
