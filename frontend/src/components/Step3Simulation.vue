...
// Format elapsed simulation time (calculated from rounds and minutes per round)
...
// Twitter platform elapsed simulation time
const twitterElapsedTime = computed(() => {
  return formatElapsedTime(runStatus.value.twitter_current_round || 0)
})

// Reddit platform elapsed simulation time
...
const doStartSimulation = async () => {
  ...
  // Reset all states first to avoid influence from previous simulation
  resetAllState()
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('Cleaned old simulation logs, restarting simulation')
      }
...
// Track last round per platform to detect changes and output logs
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect if simulation is completed (via runner_status or platform completion status)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Additional check: if backend hasn't updated runner_status yet but platforms already report completion
      // Determine by checking twitter_completed and reddit_completed status
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Incrementally add new actions (with dedup)
