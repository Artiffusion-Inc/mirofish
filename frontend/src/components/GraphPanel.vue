    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing, please refresh the graph manually later</span>
...
// Close simulation finished hint
...
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // From simulating to non-simulating state, show finished hint
...
const renderGraph = () => {
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // Self-loop edges - one merged self-loop per node
    ...
    if (totalCount > 1) {
      ...
      // Ensures all edges distribute in the same reference frame without overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add center gravity to gather isolated node clusters
  ...
  const getLinkPath = (d) => {
    ...
    if (d.isSelfLoop) {
      ...
      const y2 = sy + 4
      // Draw self-loop arc (sweep-flag=1 clockwise)
    ...
    // Calculate curve control point - dynamic based on edge count and distance
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // Perpendicular offset based on distance ratio for visible curves
  ...
  // Link labels background (white bg for clearer text)
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label position (no rotation, horizontal for clarity)
