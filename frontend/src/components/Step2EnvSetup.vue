    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create simulation instance, load simulation world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Based on context, auto-invoke tools to extract entities and relations from the knowledge graph, initialize simulation individuals, and endow them with unique behaviors and memories based on reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow rate, recommendation algorithms, active time periods, speech frequency, and event triggers based on simulation needs and reality seeds
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on narrative direction, auto-generate initial activation events and hot topics to guide the initial state of the simulation world
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation environment ready. You may start the simulation.</p>
          
          <!-- Simulation rounds config - only shown after config generation and round calculation -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds Setting</span>
                <span class="section-desc">MiroFish auto-plans reality simulation for <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of real time</span>
              </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If running for the first time, strongly recommend switching to 'Custom Mode' to reduce simulation rounds for quick preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'Generating Agent Profiles' || newStage === 'generating_profiles') {
    ...
    phase.value = 2 // Still part of config phase
  }
})
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds >= 40 (recommended) to avoid slider range issues
...
// Calculate total related topics across all profiles
...
const handleStartSimulation = () => {
  // Build params passed to parent component
  const params = {}
  ...
  if (useCustomRounds.value) {
    ...
    // User keeps auto-generated rounds, do not pass max_rounds param
    addLog(`Starting simulation, auto-configured rounds: ${autoGeneratedRounds.value} rounds`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Output detailed progress logs (avoid duplication)
        ...
        // Output message logs (avoid duplication)
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    console.warn('Fetching profiles failed:', err)
  }
}
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output config generation stage logs (avoid duplication)
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        lastLoggedConfigStage = data.generation_stage
        if (data.generation_stage === 'generating_profiles') {
          addLog('Generating agent profile config...')
        } else if (data.generation_stage === 'generating_config') {
          addLog('Calling LLM to generate simulation config params...')
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: ${tc.minutes_per_round} min/round, total ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup complete, simulation ready')
...
const loadPreparedData = async () => {
  ...
  try {
    ...
    addLog(`Loading config failed: ${err.message}`)
    emit('update-status', 'error')
  }
}
