"""
Action logger for OASIS simulations
Records each agent's actions for backend monitoring
...
class SimulationLogManager:
    ...
    Manages all log files, separated by platform
    """
...
class ActionLogger:
    """
    Action logger (backward-compatible interface)
...
# Global logger instance (backward-compatible interface)
_global_logger: Optional[ActionLogger] = None


def get_logger(log_path: Optional[str] = None) -> ActionLogger:
    """Get the global logger instance (backward-compatible interface)"""
