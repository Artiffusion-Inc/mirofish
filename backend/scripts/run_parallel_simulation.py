...
- Keeps environment alive after simulation for interview commands
- Supports IPC-based interview commands
...
# This fixes issues with OASIS third-party libraries reading files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == 'win32':
    ...
    # Reconfigure standard output streams to UTF-8 (fixes console garbled text)
    ...
    # Note: This needs to be set at Python startup; runtime changes may not take effect
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrapper for open() that defaults to UTF-8 encoding in text mode
        This fixes issues with third-party libraries (e.g., OASIS) reading files without specifying encoding
        """
...
# Load project root .env file (contains LLM_API_KEY and other config)
...
def disable_oasis_logging():
    """
    Disable OASIS library verbose logging
    OASIS logs are too verbose (records every agent observation and action), we use our own action_logger
    """
...
# Twitter available actions (INTERVIEW excluded; it can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (INTERVIEW excluded; it can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get the environment and agent_graph for a specified platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None/not specified: Interview on both platforms, return combined results

        Returns:
            True on success, False on failure
        """
        # If a platform is specified, interview only on that platform
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden by each interview item)
                - "twitter": Interview only on Twitter
                - "reddit": Interview only on Reddit
                - None/not specified: Interview each agent on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        ...
        # Split both_platforms_interviews into two platform groups
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows actions.jsonl to display real entity names instead of placeholders like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    Args:
        ...
        last_rowid: Last processed max rowid (using rowid instead of created_at to avoid format differences across platforms)
...
def _enrich_action_context(
    ...
    """
    Enrich actions with context information (post content, usernames, etc.)
    ...
    try:
        ...
        # Follow: enrich with followed user's name
        ...
        # Mute: enrich with muted user's name
        ...
        # Create comment: enrich with the commented post info
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    When boost LLM is configured, parallel simulations can use different API providers
...
def get_active_agents_for_round(
    ...
    """Decide which agents to activate for this round based on time and configuration"""
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Max simulation rounds (optional, for truncating long simulations)
    ...
    # Twitter uses general LLM configuration
    ...
    # Truncate if max rounds is specified
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end even when no agents are active (actions_count=0)
            if action_logger:
        ...
        # Fetch and log actual actions executed from the database
    ...
    # Note: Do not close the environment, keep it available for Interview
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, for truncating long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
    ...
    # Create shutdown event at the start of main so the entire program can respond to exit signals
    ...
    # Initialize logging config (disable OASIS logs, clean up old files)
    ...
    if args.max_rounds:
        ...
        if args.max_rounds < config_total_rounds:
            log_manager.info(f"  - Actual rounds to run: {args.max_rounds} (truncated)")
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-wait mode - environment kept alive")
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep so shutdown_event can be handled
                try:
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation scenario: after simulation completes, do not exit; wait for interview commands
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (prevents warnings on exit)
        try:
