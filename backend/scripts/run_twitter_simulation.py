...
This script reads parameters from a configuration file to execute the simulation, achieving full automation

Features:
- After simulation completion, do not immediately close the environment; enter command waiting mode
...
# Load the .env file from the project root (contains LLM_API_KEY and other config)
...
class MaxTokensWarningFilter(logging.Filter):
    """Filter out camel-ai warnings about max_tokens (we intentionally do not set max_tokens; let the model decide)"""

    def filter(self, record):
        # Filter log entries containing max_tokens warnings
        if "max_tokens" in record.getMessage() and "Invalid or missing" in record.getMessage():
            return False
        return True


# Add the filter immediately on module load so it takes effect before camel code runs
logging.getLogger().addFilter(MaxTokensWarningFilter())


def setup_oasis_logging(log_dir: str):
    """Configure OASIS logging using fixed log file names"""
...
class TwitterSimulationRunner:
    ...
    def __init__(self, config_path: str, wait_for_commands: bool = True):
        ...
        Args:
            config_path: Path to configuration file (simulation_config.json)
    ...
    def _create_model(self):
        ...
        Unified configuration using the .env file in the project root (highest priority):
        - LLM_API_KEY: API key
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            raise ValueError("Missing API Key configuration. Please set LLM_API_KEY in the .env file at the project root.")
    ...
    def _get_active_agents_for_round(
        ...
        # Calculate activation probability per Agent configuration
        candidates = []
...
async def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Path to configuration file (simulation_config.json)'
    )
    parser.add_argument(
        '--max-rounds',
        type=int,
    ...
    # Initialize logging config (use fixed filenames, clean up old logs)
...
def setup_signal_handlers():
    ...
    giving the program a chance to clean up resources (close database, environment, etc.)
    """
