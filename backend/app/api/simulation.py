...
# Adding this prefix prevents Agent from calling tools, responds directly with text
...
def get_graph_entities(graph_id: str):
    ...
    try:
        if Config.ZEP_BACKEND == 'cloud' and not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
                "error": "ZEP_API_KEY not configured (required for cloud mode)"
            }), 500
...
def create_simulation():
    ...
    Note: max_rounds and other parameters are intelligently generated by LLM, no manual setting needed
    
    Request (JSON):
        {
            "project_id": "proj_xxxx",      // required
            "graph_id": "mirofish_xxxx",    // optional, falls back to project if not provided
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) are kept in backend/scripts/ directory, not copied to simulation directory
    ...
    try:
        ...
        # - preparing: If config_generated=True, preparation is complete
...
def prepare_simulation():
    ...
    This is a time-consuming operation. The API will return task_id immediately.
    Use GET /api/simulation/prepare/status to query progress.
    
    Features:
    - Auto-detect completed preparation, avoid redundant generation
    - If preparation already complete, return existing results
    - Support force regeneration (force_regenerate=true)
    
    Steps:
    1. Check if preparation is already completed
    2. Read and filter entities from Zep graph
    3. Generate OASIS Agent Profile for each entity (with retry)
    4. LLM intelligently generates simulation config (with retry)
    ...
    try:
        ...
        # So frontend can get expected total agents immediately after calling prepare
        try:
            ...
            # Failure does not affect subsequent flow, background task will retry
...
def get_prepare_status():
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "progress": 45,
                "message": "...",
                "already_prepared": true|false,  // whether preparation already completed
    ...
    try:
        ...
        # If simulation_id provided, check if preparation is already complete
        if simulation_id:
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
            if is_prepared:
                return jsonify({
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": "Preparation not started yet, please call /api/simulation/prepare to begin",
                        "already_prepared": False
                    }
                })
            return jsonify({
        ...
        if not task:
            # Task not found, but if simulation_id provided, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check if generating (via state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (e.g., file modification time, whether generating)
    - Returns partial info even if config generation is not complete
    ...
    try:
        ...
        # If config exists, extract key statistics
        if config:
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (LLM-generated complete configuration)
...
# ============== Profile Generation API (standalone) ==============
...
def start_simulation():
    ...
    About force parameter:
        - When enabled, if simulation is running or completed, will first stop and clean up run logs
        - Cleaned items include: run_state.json, actions.jsonl, simulation.log, etc.
        - Will NOT clean config files (simulation_config.json) and profile files
        - Suitable for scenarios where simulation needs to be re-run

    About enable_graph_memory_update:
        - When enabled, all Agent activities (posting, commenting, liking, etc.) are updated to Zep graph in real-time
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "force_restarted": true               // whether this is a force restart
            }
        }
    """
    try:
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # Get current round actions (recent_actions shows only latest round)
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (after completing simulation loops, entering command-wait mode)
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What do you think about B?"  // if no platform specified, uses default
                }
            ],
            "platform": "reddit",              // optional, default platform (overridden by per-item platform)
                                               // When not specified: dual-platform simulation interviews both platforms per agent
    ...
    try:
        ...
        # Optimize each interview prompt, add prefix to prevent Agent from calling tools
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // required, simulation ID
            "prompt": "What do you think about this overall?",  // required, interview question (same for all agents)
...
def get_interview_history():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",          // optional, platform type (reddit/twitter)
                                           // if not specified, returns history for both platforms
            "agent_id": 0,                 // optional, get interview history for this agent only
    ...
    try:
        ...
        simulation_id = data.get('simulation_id')
        platform = data.get('platform')  # if not specified, returns history for both platforms
        agent_id = data.get('agent_id')
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Environment is running, can accept interview commands"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    Send a close environment command to the simulation for graceful exit from command-wait mode.
    
    Note: This is different from the /stop endpoint, which forcefully terminates the process.
